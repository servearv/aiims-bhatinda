    ...
    if query:
        ...
        try:
            ...
            conditions.append(
                ...
                "OR s.student_class ILIKE ? OR s.section ILIKE ? OR s.registration_number ILIKE ?)"
            )
...
@app.route("/api/students/previous-records")
def api_student_previous_records():
    ...
    matching_students = conn.execute("""
        ...
        WHERE s.registration_number = ?
          ...
          AND (?  = '' OR s.event_id != ?)
        ORDER BY e.start_date DESC
    """, (registration_number, int(school_id),
          current_event_id, int(current_event_id) if current_event_id else 0)).fetchall()
